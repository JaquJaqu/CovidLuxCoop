...
const corsFixBezirke3 = 'https://evening-reaches-25236.herokuapp.com/';
...
//ON SUCCESS
  ...
  if (!db.objectStoreNames.contains('bezirksdaten')) {
      ...
      add();
		...
		if(db.objectStoreNames.contains('bezirksdaten') && ETagBezirksdataDatabaseLS != eTagResponseBezirke3){
      clearOS();    
      updateDB();
...
//ON ERROR
     ...
     request.onupgradeneeded = function(event) {
        ...
        var objectStore = db.createObjectStore("bezirksdaten", { autoIncrement : true });checkETagBezirksDatabase(pathBezirke3);
...
//Key umbenennen --> Time zu datum
      ...
      //  console.log("du bist hier");
      //   console.log("alleBezirksdaten",alleBezirksDaten2);
      //   console.log("Bezirksdaten", BezirksDaten);
...
function meineDatenFunktion(){
  ...
  console.log("meineDatenFunktion wird ausgeführt");

  var db = event.target.result;
  var objectStore = db.transaction("bezirksdaten").objectStore("bezirksdaten");
  objectStore.getAll().onsuccess = function(event) {

    alleMeineDaten = event.target.result;
